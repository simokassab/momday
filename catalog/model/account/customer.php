...
class ModelAccountCustomer extends Model {
	...
	public function getCustomer($customer_id) {
		$query = $this->db->query("SELECT *, (SELECT COUNT(*) FROM " . DB_PREFIX . "momday_celebrity WHERE celebrity_id = customer_id" . ") AS is_celebrity FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");

		return $query->row;
	}

	public function getCustomerByEmail($email) {
		$query = $this->db->query("SELECT *, (SELECT COUNT(*) FROM " . DB_PREFIX . "momday_celebrity WHERE celebrity_id = customer_id" . ") AS is_celebrity FROM " . DB_PREFIX . "customer WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
