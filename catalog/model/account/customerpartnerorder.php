	...
	public function customerpartner($order_info,$order_status, $comment = '', $order_status_id = 0){
		...
		foreach ($order_product_query->rows as $product) {
			...
			if(isset($mpSellers['email']) AND !empty($mpSellers['email'])){
				...
				if($order_info['shipping_code'] != 'shipping_wk_custom_shipping.shipping_wk_custom_shipping' && $admin_shipping_method) {
					if(!$paid_shipping) {
					...
					$shipping_quote = $this->model_extension_shipping_wk_custom_shipping->getQuote($shipping_address, $sellerDetails);
				}

				if(isset($shipping_quote['quote']['shipping_wk_custom_shipping']['cost']) && $shipping_quote['quote']['shipping_wk_custom_shipping']['cost']) {
					if (!isset($prev_seller_id) || $prev_seller_id != $cur_seller_id) {
						$commission_array['customer'] = $commission_array['customer'] + $shipping_quote['quote']['shipping_wk_custom_shipping']['cost'];

            $shipping_applied = $shipping_quote['quote']['shipping_wk_custom_shipping']['cost'];
		...
		if($this->config->get('marketplace_mailtoseller')){
			...
			$data['title'] = sprintf($language->get('text_subject'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), $order_id);
			...
			$data['text_order_status'] = $language->get('text_order_detail');
			...
			$data['text_powered'] = $language->get('text_powered');
			...
			$textBasic = $language->get('text_order_id') . ' ' . $order_id . "\n";
			$textBasic .= $language->get('text_date_added') . ' ' . date($language->get('date_format_short'), strtotime($order_info['date_added'])) . "\n";
			$textBasic .= $language->get('text_order_status') . ' ' . $order_status . "\n\n";
