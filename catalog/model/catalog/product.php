...
class ModelCatalogProduct extends Model {
	...
	public function getProductsAllDataWithCelebritiesFilter($data = array(), $customer) {
        ...
        $sql.= $this->filterSqlWithCelebrities($data);
    ...
    private function filterSqlWithCelebrities($data, $onlycount = false){
        $sql = "";
        if (!empty($data['filter_category_id'])) {
            if (!empty($data['filter_sub_category'])) {
        ...
        if (isset($data['celebrity_id'])) {
        	$sql .= " AND EXISTS (SELECT * FROM " . DB_PREFIX . "momday_celebrity_shop AS celebrity WHERE celebrity.celebrity_id = " . $this->db->escape($data['celebrity_id']) . " AND celebrity.product_id = p.product_id)";
        }
	...
	/*
	 * Get preloved product
	 * same as get product but status can be 0 or 1
	 */
	public function getPrelovedProduct($product_id) {
	...
	public function getProducts($data = array()) {
		...
		// start momday code
		if (isset($data['momday_celebrity_id']) && !empty($data['momday_celebrity_id'])) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "momday_celebrity_shop mcs ON (mcs.product_id = p.product_id)";
        }
        // end momday code
        ...
        // start momday code
        if (isset($data['momday_celebrity_id']) && !empty($data['momday_celebrity_id'])) {
            $sql .= " AND  mcs.celebrity_id = " . (int)$data['momday_celebrity_id'];
        }
        if (isset($data['momday_new_products']) && !empty($data['momday_new_products'])) {
            $sql .= " AND p.product_id NOT IN (SELECT product_id from " . DB_PREFIX . "momday_customerseller_to_product)";
        }

        if (isset($data['momday_preloved_products']) && !empty($data['momday_preloved_products'])) {
            $sql .= " AND p.product_id IN (SELECT product_id from " . DB_PREFIX . "momday_customerseller_to_product)";
        }
        // end momday code
...
//            if(isset($data['momday_preloved_products']) && !empty($data['momday_preloved_products'])){
                $product_charity = $this->getProductCharity($result['product_id']);
                $preloved_fields = $this->getPrelovedFields($result['product_id']);
                $product_data[$result['product_id']] = array_merge($this->getProduct($result['product_id']), $product_charity);
                $product_data[$result['product_id']] = array_merge($this->getProduct($result['product_id']), $preloved_fields);
//            }else {
...
//            }
    ...
    public function getPrelovedFields($product_id) {
        $query = $this->db->query("SELECT date_expire, remarks, status as preloved_item_status, address as preloved_item_address, video FROM " . DB_PREFIX . "momday_customerseller_to_product WHERE product_id='" . $product_id . "'");
        return $query->row;
    }

    public function getProductCharity($product_id) {
        $query = $this->db->query("SELECT charity_id FROM " . DB_PREFIX . "momday_product_to_charity WHERE product_id='" . $product_id . "'");
        return $query->row;
    }

    public function getPrelovedProductIds(){
        $query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "momday_customerseller_to_product");
        return $query->rows;
    }
    // end momday code
}
