...
class ControllerAccountCustomerpartnerCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('account/customerpartner/category', $url, true));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('account/customerpartner/category/add', $url, true);
		$data['delete'] = $this->url->link('account/customerpartner/category/delete', $url, true);
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('account/customerpartner/category/edit', 'category_id=' . $result['category_id'] . $url, true),
				'delete'      => $this->url->link('account/customerpartner/category/delete', 'category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('account/customerpartner/category', 'sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('account/customerpartner/category', 'sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('account/customerpartner/category', $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('account/customerpartner/category_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('account/customerpartner/category/add', $url, true);
		} else {
			$data['action'] = $this->url->link('account/customerpartner/category/edit', 'category_id=' . $this->request->get['category_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('account/customerpartner/category', $url, true);
		...
		if (isset($this->request->post['category_seo_url'])) {
			...
			$data['category_seo_url'] = $this->model_customerpartner_category->getCategorySeoUrls($this->request->get['category_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('account/customerpartner/category_form', $data));
	}
			...
			foreach ($this->request->post['category_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if (!empty($keyword)) {
						...
						$seo_urls = $this->model_customerpartner_category->getSeoUrlsByKeyword($keyword);
