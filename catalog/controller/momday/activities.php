...
class ControllerMomdayActivities extends Controller {
    ...
    public function __construct($registry) {
        ...
        $this->language->load('momday/activities');
...
//        }
    ...
    public function index() {
        ...
        $this->data['breadcrumbs'][] = array(
            'text'      => $this->language->get('text_momday_activities'),
            'href'      => $this->url->link('momday/activities'),
        ...
        if ($category_info) {
            ...
            $this->data['breadcrumbs'][] = array(
                ...
                'href'      => $this->url->link('momday/activities', 'journal_blog_category_id=' . $category_id . $url),
                'separator' => $this->language->get('text_separator')
            );
        ...
        if ($this->journal2->settings->get('config_blog_settings.feed', 1)) {
            $this->journal2->settings->set('blog_blog_feed_url', $this->url->link('momday/activities/feed', $category_info ? 'journal_blog_feed_category_id=' . $category_id : ''));
        }

        $this->data['heading_title'] = $this->language->get('momday_activities_heading_title');
        $this->document->setTitle($this->language->get('text_momday_activities'));
        ...
        $all_host_addresses = $this->model_momday_activities->getAllActivityHostAddresses();
        $activity_id_to_location = $this->get_activity_id_to_host_address($all_host_addresses);

        $this->data['activity_id_to_location'] = $activity_id_to_location;
        $this->data['location_image'] = $this->get_base_url() . 'image/momday/activities/location-sm.png';

        $posts = $this->model_momday_activities->getPosts($data);
        $posts_total = $this->model_momday_activities->getPostsTotal($data);
        ...
        foreach ($posts as $post) {
            ...
            $this->data['posts'][] = array(
                ...
                'href'          => $this->url->link('momday/activities/post', ($category_info ? 'journal_blog_category_id=' . $category_id . '&' : '') . 'activity_id=' . $post['post_id']),
        ...
        $pagination->url = $this->url->link('momday/activities', $url . '&page={page}');
        ...
        $this->blog_template = 'momday/activity_posts.tpl';
    ...
    public function post() {
        ...
        if ($category_info) {
            $this->data['breadcrumbs'][] = array(
                'text'      => $category_info['name'],
                'href'      => $this->url->link('momday/activities', 'journal_blog_category_id=' . $category_id),
                'separator' => $this->language->get('text_separator')
            );
        }

        if (isset($this->request->get['activity_id'])) {
            $post_id = $this->request->get['activity_id'];
        } else {
            $post_id = 0;
        }

        if($this->check_post_is_activity($post_id)) {
        ...
        if ($post_info) {
            $this->data['breadcrumbs'][] = array(
                'text'      => $post_info['name'],
                'href'      => $this->url->link('momday/activities/post', ($category_info ? 'journal_blog_category_id=' . $category_id . '&' : '') . 'activity_id=' . $post_info['post_id']),
                'separator' => $this->language->get('text_separator')
            );


            $this->data['host_details'] = $this->get_activity_host_details($post_id);
            $this->data['location_image'] = $this->get_base_url() . 'image/momday/activities/location-sm.png';
            $this->data['email_image'] = $this->get_base_url() . 'image/momday/activities/email-sm.png';
            $this->data['phone_image'] = $this->get_base_url() . 'image/momday/activities/phone-sm.png';
            $this->data['website_image'] = $this->get_base_url() . 'image/momday/activities/website-sm.png';
            ...
            foreach (explode(',', $post_info['tags']) as $tag) {
                ...
                if (!$tag) continue;
                $this->data['post_tags'][] = array(
                    'href'  => $this->url->link('momday/activities', 'journal_blog_tag=' . $tag),
                    'name'  => $tag
                );
            }
            ...
            foreach ($results as $result) {
                $this->data['post_categories'][] = array(
                    'href'  => $this->url->link('momday/activities', 'journal_blog_category_id=' . $result['category_id']),
                    'name'  => $result['name']
                );
            }
            ...
            $this->document->setDescription($this->blog_meta_description);
            $this->document->setKeywords($this->blog_meta_keywords);
            $this->document->addLink($this->url->link('journal2/blog/post', 'activity_id=' . $post_id), 'canonical');
            $this->journal2->settings->set('blog_meta_title',       $this->blog_meta_title);

            $this->blog_template = 'momday/activity_post.tpl';
        } else {
    ...
    public function feed() {
        ...
        $output .= '<channel>';
        $output .= '<atom:link href="' . $this->url->link('momday/activities/feed') . '" rel="self" type="application/rss+xml" />';
        $output .= '<title>' . $this->blog_title . '</title>';
        $output .= '<link>' . $this->url->link('momday/activities') . '</link>';
        ...
        foreach ($this->model_journal2_blog->getPosts($data) as $post) {
            ...
            $output .= '<link>' . $this->url->link('momday/activities/post', 'activity_id=' . $post['post_id']) . '</link>';
            $output .= '<guid>' . $this->url->link('momday/activities/post', 'activity_id=' . $post['post_id']) . '</guid>';
            ...
            $description .= '<a href="' . $this->url->link('momday/activities/post', 'activity_id=' . $post['post_id']) . '">' . $this->journal2->settings->get('blog_button_read_more', 'Read More') .'</a>';
    ...
    private function get_activity_id_to_host_address($all_activity_addresses){
        $activity_id_to_host_address = array();

        foreach ($all_activity_addresses as $result) {
            $activity_id_to_host_address[$result['post_id']]= $result['location'];
        }
        return $activity_id_to_host_address ;
    }

    private function get_activity_host_details($post_id){
        $host_details = $this->model_momday_activities->getActivityHostDetails($post_id);
        $host_info = array();
        if(sizeof($host_details) >= 1) {
            $host_details = $host_details[0];
            $host_info =array(
                'location' => $host_details['location'],
                'phone' => $host_details['phone'],
                'email' => $host_details['email'],
                'website' => $host_details['website']
            );
        }
