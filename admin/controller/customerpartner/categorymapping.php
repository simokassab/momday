...
class ControllerCustomerpartnerCategorymapping extends Controller {
	...
	private $data = array();
  	...
  	public function index() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customerpartner/categorymapping', 'user_token=' . $this->session->data['user_token'] . $url, true),
      		'separator' => ' :: '
   		);

		$data['insert'] = $this->url->link('customerpartner/categorymapping/add', 'user_token=' . $this->session->data['user_token'] . $url . '&mpcheck=1', true);

		$data['delete'] = $this->url->link('customerpartner/categorymapping/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		if ($results) {
			foreach ($results as $result) {
	      	...
	      	$data['categories'][] = array(
					...
					'action'	=> $this->url->link('customerpartner/categorymapping/add', 'user_token=' . $this->session->data['user_token'] . '&category_id=' . $result['category_id'] .'&attribute_id='.$result['attribute_id'] , true)
				);
	    }
		}
		...
		$pagination->url = $this->url->link('customerpartner/categorymapping', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customerpartner/categorymapping_list',$data));
  	}
  	...
  	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('customerpartner/categorymapping', 'user_token=' . $this->session->data['user_token'], true));
		}
    	...
    	$data['cancel'] = $this->url->link('customerpartner/categorymapping', 'user_token=' . $this->session->data['user_token'] . $url, true);

		$data['save'] = $this->url->link('customerpartner/categorymapping/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('customerpartner/categorymapping_form',$data));
	}
