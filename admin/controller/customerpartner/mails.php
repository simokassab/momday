...
class Controllercustomerpartnermails extends Controller {
	...
	private $data = array();
  	...
  	public function getlist() {
		...
		foreach ($filter_array as $unsetKey => $key) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token']. $url, true),
      		'separator' => ' :: '
   		);

		$this->data['delete'] = $this->url->link('customerpartner/mails/delete', 'user_token=' . $this->session->data['user_token'] , true);
		$this->data['insert'] = $this->url->link('customerpartner/mails/addmail', 'user_token=' . $this->session->data['user_token'] , true);
	    ...
	    foreach ($results as $result) {
	      	...
	      	$this->data['mails'][] = array(
				...
				'action' => $this->url->link('customerpartner/mails/addMail', 'user_token=' . $this->session->data['user_token'] . '&mail_id=' . $result['id'], true),
			);
		...
		$this->data['sort_name'] = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$this->data['sort_id'] = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . '&sort=id' . $url, true);
		$this->data['sort_message'] = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . '&sort=message' . $url, true);
		$this->data['sort_subject'] = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . '&sort=subject' . $url, true);
		...
		$pagination->url = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customerpartner/mail_list',$this->data));
  	}
  	...
  	public function addmail() {
    	...
    	$this->data['cancel'] = $this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$this->data['save'] = $this->url->link('customerpartner/mails/mailSave', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->data['footer'] = $this->load->controller('common/footer');
		$this->data['column_left'] = $this->load->controller('common/column_left');
		$this->response->setOutput($this->load->view('customerpartner/mail_form',$this->data));
	...
	public function mailSave() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(!isset($this->error['warning'])){
				...
				$this->response->redirect($this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'], true));
			}
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('customerpartner/mails', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
