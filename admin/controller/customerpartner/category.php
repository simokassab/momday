...
class ControllerCustomerpartnerCategory extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('customerpartner/category', 'user_token=' . $this->session->data['user_token'] . $url, true),
              'separator' => ' :: '
        );

        $data['insert'] = $this->url->link('customerpartner/category/add', 'user_token=' . $this->session->data['user_token'] . $url . '&mpcheck=1', true);

        $data['delete'] = $this->url->link('customerpartner/category/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        if ($results) {
            foreach ($results as $result) {
                $data['categories'][] = array(
                    ...
                    'action'    => $this->url->link('customerpartner/category/add', 'user_token=' . $this->session->data['user_token'] . '&seller_id=' . $result['seller_id'], true)
                );
            }
        }
        ...
        $data['sort_seller_name'] = $this->url->link('customerpartner/category', 'user_token=' . $this->session->data['user_token'] . '&sort=c.seller_id' . $url, true);
        ...
        $pagination->url = $this->url->link('customerpartner/category', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('customerpartner/category_list', $data));
    }
    ...
    public function add()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->url->link('customerpartner/category', 'user_token=' . $this->session->data['user_token'], true));
        }
        ...
        $data['cancel'] = $this->url->link('customerpartner/category', 'user_token=' . $this->session->data['user_token'] . $url, true);

        $data['save'] = $this->url->link('customerpartner/category/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
        ...
        $this->response->setOutput($this->load->view('customerpartner/category_form', $data));
    }
