...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			$product_query = $this->db->query("SELECT p2s.*, p.*, pd.*, m.name AS manufacturer_name, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_to_store p2s LEFT JOIN " . DB_PREFIX . "product p ON (p2s.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p2s.product_id = '" . (int)$cart['product_id'] . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1'");
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_data[] = array(
					...
					'manufacturer'	  => $product_query->row['manufacturer_name'],
					'rating'		  => $product_query->row['rating'],
					'option'          => $option_data,
